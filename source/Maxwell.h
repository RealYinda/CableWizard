...
public:
    ...
    void source(double x, double y, double z, TYPE* value) const {
        value[0] = 0.0;
    }
    void boundary_1(double x, double y, double z, TYPE* value) const {
        value[0] = 0.0;
    }
...
: public algs::StandardComponentPatchStrategy<NDIM>
{
 public:
   ...
   *  -# 定义变量和数据片.
   */
  Maxwell(const string& object_name,
   ...
   */
  void reduceOnPatch(double *vector,
                     int     len,
...
private:
  ...
  void buildLHSAndRHSOnPatch(hier::Patch<NDIM> & patch);
